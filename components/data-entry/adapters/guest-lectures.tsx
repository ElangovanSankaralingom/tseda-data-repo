  ...
  function renderSavedEntry(entry: GuestLectureEntry, category: EntryDisplayCategory, index: number) {
    ...
    return (
      <CategoryEntryRecordCard
        category={category}
        index={index}
        href={entryDetail("guest-lectures", entry.id)}
        title={entry.eventName}
        streakState={getEntryStreakDisplayState(entry)}
        deadlineState={deadlineState}
        confirmationStatus={confirmationStatus}
        subtitle={`Speaker: ${entry.speakerName} • ${entry.organizationName}`}
        createdAt={entry.createdAt}
        updatedAt={entry.updatedAt}
        onView={() => router.push(entryDetail("guest-lectures", entry.id))}
        onPreview={
          entry.pdfMeta?.url
            ? () => window.open(entry.pdfMeta?.url, "_blank", "noopener,noreferrer")
            : undefined
        }
        onEdit={lockApproved ? undefined : () => router.push(entryDetail("guest-lectures", entry.id))}
        onDelete={
          lockApproved
            ? undefined
            : () =>
