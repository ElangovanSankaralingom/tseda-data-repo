...
export function FdpConductedPage({
  ...
  const controller = useCategoryEntryPageController<FdpConducted>({
    ...
    persistRequestEdit: async (entry) => {
      const response = await fetch("/api/me/entry/confirmation", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ categoryKey: "fdp-conducted", entryId: entry.id }),
      });
  ...
  async function persistCoCoordinatorRows(nextRows: FacultyRowValue[]) {
    return persistCurrentMutation({
      buildNextEntry: (current) => ({
        ...current,
        coordinatorName: currentFaculty?.name ?? current.coordinatorName,
        coordinatorEmail: currentFaculty?.email ?? current.coordinatorEmail,
  ...
  const renderSavedEntry = createCategoryEntryRecordRenderer<FdpConducted>({
    ...
    canFinalise: (entry) => {
      ...
      const progress = computeFieldProgress("fdp-conducted", entry as Record<string, unknown>);
    ...
    <CategoryEntryRuntime
      ...
      headerActions={getHeaderActionProps({
        ...
        onFinalise: isViewMode && isEntryEditable(form) && (() => {
          const progress = computeFieldProgress("fdp-conducted", form as Record<string, unknown>);
          return progress.total > 0 && progress.completed === progress.total;
