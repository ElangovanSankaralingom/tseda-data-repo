...
function getStoreFile(email: string) {
  return path.join(STORE_ROOT, sanitizeSegment(email), "guest-lectures.json");
}
...
export async function POST(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const organizationName = String(entry.organizationName ?? "").trim();
  const studentYear = String(entry.studentYear ?? "").trim();
  const semesterNumber = entry.semesterNumber == null ? "" : String(entry.semesterNumber);

  if (!academicYear || !semesterType || !startDate || !endDate || !eventName || !speakerName || !organizationName || !studentYear || !semesterNumber) {
  ...
  const bytes = await generateEntryPdfBytes({
    ...
    fields: [
      ...
      { label: "Student Year", value: studentYear },
      { label: "Semester", value: semesterNumber },
  ...
  const pdfMeta = await storeEntryPdf({
    email,
    categoryFolder: "guest-lectures",
    entryId,
    fileNameBase: eventName || "guest-lecture-entry",
    bytes,
  });
  ...
  const updatedEntry: EntryRecord = {
    ...
    pdfMeta,
    pdfSourceHash: hashPrePdfFields(entry as Record<string, unknown>, "guest-lectures"),
    pdfStale: false,
