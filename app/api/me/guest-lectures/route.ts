...
function buildSavedStreak(
  entry: Pick<GuestLectureEntry, "status" | "pdfMeta" | "startDate" | "endDate" | "streak" | "uploads">
) {
...
function getStoreFile(email: string) {
  return path.join(process.cwd(), ".data", "users", safeEmailDir(email), "guest-lectures.json");
}
...
async function readList(email: string): Promise<GuestLectureEntry[]> {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.map(normalizeEntry).filter((entry): entry is GuestLectureEntry => !!entry)
      : [];
  } catch {
    return [];
  }
    ...
    for (const target of targets) {
      ...
      const clonedEntry: GuestLectureEntry = {
        ...
        uploads: {
          ...
          speakerProfile: await cloneFileMetaToTarget(
            savedEntry.uploads.speakerProfile!,
            target.email,
            "guest-lectures",
            sharedEntryId,
            "speakerProfile"
