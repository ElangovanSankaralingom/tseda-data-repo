...
async function writeList(email: string, list: CaseStudyRecord[]) {
...
function getSlotFromStoredPath(storedPath: string): Slot | null {
  ...
  return slot === "permissionLetter" || slot === "travelPlan" || slot === "geotaggedPhotos" ? slot : null;
}
...
export async function POST(request: Request) {
  ...
  try {
    enforceRateLimitForRequest({
      ...
      action: "upload.case-studies.post",
    ...
    if (file instanceof File) {
      assertUploadMetadataInput(
        ...
        "case-studies upload request"
      );
    } else {
      assertUploadMetadataInput({ email, recordId, slot }, "case-studies upload request");
    }
...
export async function DELETE(request: Request) {
  ...
  try {
    enforceRateLimitForRequest({
      request,
      userEmail: authorizedEmail,
      action: "upload.case-studies.delete",
      options: RATE_LIMIT_PRESETS.uploadOps,
    });
    ...
    assertUploadMetadataInput(
      ...
      "case-studies upload delete request"
    );
