...
import { cancelEditRequest, requestEdit } from "@/lib/entries/lifecycle";
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    assertActionPayload(body, "cancel edit request", SECURITY_LIMITS.actionPayloadMaxBytes);
    const categoryKey = String(body?.categoryKey ?? "").trim();
    const entryId = String(body?.entryId ?? body?.id ?? "").trim();
    ...
    const updatedEntry = await cancelEditRequest(email, categoryKey, entryId);
    revalidatePath(dashboard());
    revalidatePath(dataEntryHome());
    revalidatePath(entryList(categoryKey));
    revalidatePath(entryDetail(categoryKey, entryId));
    ...
    const appError = normalizeError(error);
    logError(appError, "api.me.entry.cancelEditRequest.DELETE");
