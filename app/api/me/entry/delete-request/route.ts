...
import { cancelDeleteRequest, requestDelete } from "@/lib/entries/lifecycle";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    assertActionPayload(body, "delete request", SECURITY_LIMITS.actionPayloadMaxBytes);
    ...
    const updatedEntry = await requestDelete(email, categoryKey, entryId, message || undefined);
    ...
    logError(appError, "api.me.entry.requestDelete.POST");
    ...
    if (appError.message.startsWith("Invalid status transition:")) {
      return NextResponse.json(
        { error: "Entry cannot request deletion in the current state." },
        { status: 400 }
      );
    }
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    assertActionPayload(body, "cancel delete request", SECURITY_LIMITS.actionPayloadMaxBytes);
    const categoryKey = String(body?.categoryKey ?? "").trim();
    const entryId = String(body?.entryId ?? body?.id ?? "").trim();
    ...
    const updatedEntry = await cancelDeleteRequest(email, categoryKey, entryId);
    revalidatePath(dashboard());
    revalidatePath(dataEntryHome());
    revalidatePath(entryList(categoryKey));
    revalidatePath(entryDetail(categoryKey, entryId));
    ...
    const appError = normalizeError(error);
    logError(appError, "api.me.entry.cancelDeleteRequest.DELETE");
    ...
    if (appError.message.startsWith("Invalid status transition:")) {
      return NextResponse.json(
        { error: "Entry is not in a cancellable state." },
        { status: 400 }
      );
    }
    if (appError.code === "RATE_LIMITED") {
